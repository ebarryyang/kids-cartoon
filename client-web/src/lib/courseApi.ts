...
const STATIC_COURSES_JSON = '/data/courses.json';

let staticCache: { series: CourseMaterial[] } | null = null;
async function loadStaticCourses(): Promise<{ series: CourseMaterial[] } | null> {
  if (staticCache) return staticCache;
  try {
    const resp = await fetch(STATIC_COURSES_JSON, { cache: 'force-cache' });
...
function staticFindSeries(series: CourseMaterial[], id: string) {
  return series.find(s => String(s.seriesId) === String(id)) || null;
}
function staticFindEpisode(s: CourseMaterial, epId: string) {
  return (Array.isArray(s.episodes) ? s.episodes : []).find(e => String(e.episodeId) === String(epId)) || null;
}
...
export const getCourseList = async (): Promise<{ success: boolean; data: { seriesId: string; seriesName: string; coverUrl?: string; episodeCount: number }[] }> => {
  try {
    ...
    console.warn('[courseApi] getCourseList axios failed, fallback to static', err);
    const s = await loadStaticCourses();
    if (!s) throw err;
    return {
      success: true,
      data: s.series.map(x => ({
        seriesId: x.seriesId, seriesName: x.seriesName,
        coverUrl: x.coverUrl || '',
        episodeCount: Array.isArray(x.episodes) ? x.episodes.length : 0,
      })),
    };
  }
};

// 获取课程详情（含所有单集资料）— axios 失败立刻 fallback 读 /data/courses.json（B 分支 courses 播放不依赖 API 也能跑）
export const getCourseDetail = async (seriesId: string): Promise<{ success: boolean; data: CourseMaterial }> => {
  try {
    ...
    console.warn('[courseApi] getCourseDetail axios failed, fallback to static /data/courses.json seriesId=', seriesId);
    const s = await loadStaticCourses();
    if (!s) throw err;
    const found = staticFindSeries(s.series, seriesId);
    if (!found) return { success: false, data: null as any };
    return { success: true, data: found };
  }
};

// 获取单集资料（同样 static fallback）
export const getEpisodeMaterial = async (seriesId: string, episodeId: string): Promise<{ success: boolean; data: EpisodeMaterial }> => {
  try {
    ...
    console.warn('[courseApi] getEpisodeMaterial axios failed, fallback to static seriesId=', seriesId, 'episodeId=', episodeId);
    const s = await loadStaticCourses();
    if (!s) throw err;
    const series = staticFindSeries(s.series, seriesId);
    if (!series) return { success: false, data: null as any };
    const ep = staticFindEpisode(series, episodeId);
    if (!ep) return { success: false, data: null as any };
    return { success: true, data: ep };
  }
};

// 生词气泡事件（H5 TimelineEvent / 小程序 CourseEvent 通用格式）
...
// 解析 vocabulary JSON URL（可能是 纯数组 / 对象 events / Markdown）
export const loadVocabularyEvents = async (url: string): Promise<VocabularyEvent[]> => {
  if (!url) return [];
  try {
    const resp = await axios.get(url, { responseType: 'text', timeout: 15000 });
    const raw: string = typeof resp.data === 'string' ? resp.data : JSON.stringify(resp.data);
    const text = (raw || '').trim();
    if (!text) return [];

    // 1) JSON: 对象 { events: [...] } 或纯数组
    if (text.startsWith('{') || text.startsWith('[')) {
      const obj = JSON.parse(text);
      if (Array.isArray(obj)) return normalizeEvents(obj);
      if (obj && Array.isArray((obj as VocabularyPayload).events)) {
        return normalizeEvents((obj as VocabularyPayload).events);
      }
      return [];
    }

    // 2) Markdown 表格（老格式）
    const mdParsed = parseMarkdownVocabulary(text);
    if (mdParsed.length > 0) return mdParsed;

    return [];
  } catch (err) {
    console.warn('[courseApi] loadVocabularyEvents failed for', url, err);
    return [];
  }
};

function normalizeEvents(arr: any[]): VocabularyEvent[] {
  const raw: VocabularyEvent[] = [];
  arr.forEach((v, i) => {
    if (!v) return;
    ...
    raw.push({
      id: String(v.id ?? `evt_loaded_${i + 1}`),
  ...
  // 💥 前端统一 POS 过滤：最后兜底只保留 名词 + 动词（3 层递进）
  return filterNounVerbOnly(raw).sort((a, b) => a.time - b.time);
}

// ============================================================
// 💥 前端 POS 过滤：只留 名词(noun) + 动词(verb)，算法与 Python 侧 auto_process_video.py 完全对齐
//   ① STOPWORDS + 形/副/介/连/助动词 黑名单 → ② 带 pos 字段直接信它 → ③ 白名单 + 后缀启发式
// ============================================================
...
function isLikelyNoun(low: string): boolean {
...
function isLikelyVerb(low: string): boolean {
...
export function filterNounVerbOnly(events: VocabularyEvent[]): VocabularyEvent[] {
  if (!events || events.length === 0) return [];
  const seen = new Set<string>();
  const out: VocabularyEvent[] = [];
  for (const e of events) {
    const en = (e?.wordEn || '').trim();
    if (!en) continue;
    const low = en.toLowerCase();
    if (seen.has(low)) continue;
    if (ENGLISH_STOPWORDS.has(low) || POS_BLACKLIST_EXTRA.has(low)) continue;
    // 任何一个对象 (events 可能带 pos) → 信 pos
    const pos = String((e as any).pos || '').toLowerCase().trim();
    let keep = false;
...
function parseMarkdownVocabulary(text: string): VocabularyEvent[] {
  ...
  return normalizeEvents(rows.map(cols => ({
